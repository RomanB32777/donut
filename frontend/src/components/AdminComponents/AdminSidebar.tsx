...
const AdminSidebar: FC<IAdminSidebar> = ({ collapsed, sidebarStyles, setCollapsed }) => {
	...
	const menuItems: IRoute[] = useMemo(() => {
		const addToMenu = (route: IRoute, menuArr: IRoute[], roleplay: any, iter = 0) => {
			const add = () => {
				if (route.children) {
					iter++
		...
		const menuShowItems = routers.reduce((acc, route) => {
			route.protected ? id && addToMenu(route, acc, roleplay) : addToMenu(route, acc, roleplay)
			return acc
		}, [] as IRoute[])
	...
	const activeRoute = useMemo(() => {
		...
		return pathElements.includes('widgets') ? `widgets/${pathElements[0]}` : pathElements[0]
	}, [pathname])
	...
	return (
		...
		>
			{isSupporter && isMobile && (
				<WalletBlock modificator="sidebar-wallet" popupModificator="wallet-popup" />
			)}
		</Sidebar>
	)
}
