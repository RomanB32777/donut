...
import { FileUploadTypes, IEditCreator, IShortUserData, IUser, userRoles } from 'types'
...
const userApi = createApi({
	...
	endpoints: (build) => ({
		...
		checkIsExistUserRole: build.query<boolean, { role: userRoles; field: string }>({
		...
		createUser: build.mutation<IUser, Omit<IShortUserData & IsVisibleNotification, 'id'>>({
		...
		editUser: build.mutation<IUser, Partial<IEditUserInfo & IsVisibleNotification>>({
			query: ({ isVisibleNotification, ...userInfo }) => {
