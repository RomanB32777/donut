...
const DonationsContainer = () => {
	...
	const [queryForm, setQueryForm] = useState<donationsQueryData<periodItemsTypes>>({
		timePeriod: '7days',
		searchStr: '',
		groupByName: false,
		startDate: '',
	...
	const selectTableData = useMemo(
		() =>
			createSelector(
				...
				(res: IDonationWidgetInfo[] | undefined, roleplay: userRoles) => roleplay,
				(data, roleplay) => {
					if (data) {
						const forTableData = data.reduce<ITableData[]>(
							(
								...
								const name = roleplay === 'backers' ? creator?.username : backer?.username
								data.push({
									key: id,
									name: name ?? username,
									donationToken: blockchainSum ? +formatNumber(blockchainSum) : 0,
	...
	const isCreator = useMemo(() => roleplay && roleplay === 'creators', [roleplay])
		...
		<div className="donations-container fadeIn">
								...
								{isCreator && (
									<Col md={13} xs={12}>
										<FormInput
											name="widgetTitle"
											value={searchStr || ''}
											setValue={(value) => setQueryForm({ ...queryForm, searchStr: value })}
			...
			{visibleDatesPicker && (
				<div className="selectDates fadeIn">
					...
					<DatesPicker
						setValue={(startDate, endDate) => setQueryForm({ ...queryForm, startDate, endDate })}
					/>
				</div>
			)}
