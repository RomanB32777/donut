...
const HelpCenterContainer = () => {
	...
	const activeTheme = useMemo(() => {
		...
		return subThemes.find(({ key }) => key.split('center/')[1] === theme) || subThemes[0]
	}, [itemsMenu, theme])

	const defaultOpenKey = useMemo(
		() => itemsMenu.find((item) => item.children.some((theme) => theme.key === activeTheme.key)),
	...
	return (
		<div className="helpCenter">
			...
			>
				...
				{activeTheme.content?.map((item, index) => renderContent({ content: item, index }))}
			</div>
		</div>
	)
}
