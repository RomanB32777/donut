...
const AuthModal: FC<IAuthModal> = ({ isOpenModal, closeModal, registrationRoleUser }) => {
	const [modalType, setModalType] = useState<AuthModalTypes>('registration')

	const changeTypeModal = useCallback((type: AuthModalTypes) => setModalType(type), [])
	...
	return (
		...
		>
			{modalType === 'registration' && registrationRoleUser ? (
				<RegistrationModal changeTypeModal={changeTypeModal} roleplay={registrationRoleUser} />
			) : (
