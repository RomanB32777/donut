...
import { RoutePaths, initAlertData, baseURL, widgetVideoLinks } from 'consts'
...
const AlertsContainer = () => {
	...
	const [editAlert, { isLoading: isEditLoading }] = useEditAlertsWidgetMutation()

	const { id: widgetID } = formData

	const sendData = (isReset?: boolean) => async () => {
		const { banner, messageFont, nameFont, sumFont, sound, ...alertData } = formData
	...
	return (
		<div className="alerts-container fadeIn">
			...
			<div className="link-top">
				...
				<LinkCopy link={linkForStream} isSimple={!isLaptop} title={renderLinkForStream} />
			</div>
			<VideoBlock link={widgetVideoLinks.alert} />
			<div className="alertsSettings">
				...
				{isGetLoading || !formData.id ? (
					...
					<WidgetMobileWrapper
						previewBlock={<PreviewAlertsBlock key="preview" formData={formData} />}
						settingsBlock={
							<SettingsAlertsBlock
								key="settings"
								fonts={fonts}
