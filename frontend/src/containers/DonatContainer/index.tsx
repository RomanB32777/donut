...
import { LoadingModalComponent, SuccessModalComponent } from 'components/ModalComponent'
...
const requiredFormFields: sendDonatFieldsKeys[] = ['sum', 'username', 'message']
...
const DonatContainer = () => {
	...
	const { username, message, sum, blockchain: selectedBlockchain, isAnonymous } = form
	...
	const sendBtnHandler = useCallback(async () => {
		...
		const notValidFields = keys.filter((key) => {
			if (requiredFormFields.includes(key as any)) return !form[key]
			return false
		})
	...
	const isNotValidAmountField = useMemo(() => notValidFields.includes('sum'), [notValidFields])
	...
	const { creator, avatarLink, walletAddress, username: personUsername } = personInfo
	...
	if (!walletAddress) {
		return (
			<h3 style={{ textAlign: 'center' }}>
				<FormattedMessage id="donat_not_connected" values={{ personUsername }} />
			</h3>
		)
	}

	const { backgroundBanner, headerBanner, backgroundColor, mainColor, welcomeText, btnText } =
		creator
	...
	return (
		...
		>
			<HeaderComponent
				logoUrl={id ? `/${RoutePaths.admin}/${RoutePaths.donations}` : RoutePaths.main}
			...
			<div className="donat-container">
								...
								<div className="inputs">
									<div className="item">
										<FormInput
											...
											disabled={Boolean(usernameState) || isBalanceLoading || isLoading}
											...
											addonAfter={
												<div className="username-switch">
													<SwitchForm
														label={
															!isMobile ? <FormattedMessage id="donat_form_switch_label" /> : ''
														}
														checked={isAnonymous}
														setValue={(flag) => setForm({ ...form, isAnonymous: flag })}
														labelModificator="switch-label"
														maxWidth={250}
														labelCol={18}
														switchCol={6}
														gutter={[0, 18]}
														tooltipTitle={<FormattedMessage id="donat_form_switch_label" />}
													/>
												</div>
											}
										/>
									...
									<div className="item">
										<FormInput
											value={message}
											setValue={(message) => formHandler({ field: 'message', value: message })}
				...
				<SuccessModalComponent
					...
					description={<FormattedMessage id="donat_success_message_description" />}
				/>
			</div>
		</div>
	)
