...
import { filterCurrentPeriodItems, filterDataTypeItems, filterPeriodItems } from 'consts'
...
export const DateFormatter = (date: string, toFormat = 'DD/MM/YYYY HH:mm') => {
...
// period-time
export const getTimePeriodQuery: (timePeriod: string) => periodItemsTypes = (timePeriod) => {
...
// period-current-time
export const getCurrentTimePeriodQuery: (timePeriod: string) => string = (timePeriod) => {
	const findKey = Object.keys(filterCurrentPeriodItems).find((key) => key === timePeriod)
...
export const getStatsDataTypeQuery: (dataType: string) => string = (
	...
	const findKey = Object.keys(filterDataTypeItems).find((key) => key === dataType)
