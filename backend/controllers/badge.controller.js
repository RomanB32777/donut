...
class BadgeController {
  ...
  async getBadge(req, res) {
    try {
      ...
      const badges = await db.query(
        `SELECT * FROM badges WHERE id = $1 AND contract_address= $2`,
  ...
  async deleteBadge(req, res) {
    try {
      const { badge_id, contract_address } = req.params;
      const deletedBadge = await db.query(
        `DELETE FROM badges WHERE id = $1 AND contract_address= $2 RETURNING *;`,
        [badge_id, contract_address]
      );
  ...
  async assignBadge(req, res) {
    try {
      ...
      const contributor_user_id_list =
        existingBadge.rows[0].contributor_user_id_list;
  ...
  async getBadgesHolders(req, res) {
    try {
      ...
      if (users.rows && users.rows[0].contributor_user_id_list.length) {
        const usersIDs = users.rows[0].contributor_user_id_list
          .split(" ")
          .filter(Boolean)
          .join(",");
  ...
  async getBadgesByCreator(req, res) {
    try {
      ...
      const badges = await db.query(
        "SELECT * FROM badges WHERE owner_user_id = $1",
        [parseInt(user_id)]
      );
