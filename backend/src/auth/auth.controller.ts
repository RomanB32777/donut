...
export class AuthController {
	...
	registration(@Req() req: Request, @Body() createUserDto: CreateUserDto) {
		...
		const confirmLink = `${req.get('Origin') || this.clientUrl}${req.originalUrl.replace(
			'registration',
			'confirm',
		)}`
	...
	resendConfirmLetter(@Req() req: Request, @Body() { email }: EmaiBodyDto) {
		const confirmLink = `${req.protocol}://${req.get('Host')}${req.originalUrl.replace(
			'resend',
			'confirm',
		)}`
	...
	async checkToken(@Param('tokenId') tokenId: string) {
		const { access_token, verify_data } = await this.authService.checkToken(tokenId)
