...
export class BadgesService {
	...
	async findOne(id: string, user?: User) {
		...
		if (badge) {
			...
			if (roleplay === 'backers' && tokenId) {
				const { contract } = await this.getWalletInfo()
				const contractBalance = await contract.balanceOf(walletAddress, tokenId)
	...
	async getAssignPrice({ userAddress, tokenId }: AssignBadgeDto) {
		...
		const mintGasCount = await contract.estimateGas.mint(userAddress, tokenID, 1, [])

		if (gasPrice && mintGasCount) {
			const price = (gasPrice.toNumber() * mintGasCount.toNumber()) / 1e18
			return price
		}
		throw new HttpException('Get price error', HttpStatus.INTERNAL_SERVER_ERROR)
	}
