...
export class AlertsService {
	...
	async update(user: User, id: string, updateAlertDto: UpdateAlertDto, file?: Express.Multer.File) {
		const { id: userId } = user
		const { isReset, ...alertData } = updateAlertDto

		if (isReset) {
			const columns = getDefaultValues(this.alertsRepository)
			await this.alertsRepository.update({ id, creator: { id: userId } }, { ...columns })
			...
			await this.alertsRepository.update({ id, creator: { id: userId } }, { ...alertData })
		}
