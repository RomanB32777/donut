...
export class ExchangeService {
	...
	async addExchange(createExchangeDto: CreateExchangeDto | CreateExchangeDto[]) {
		...
		const { raw } = await this.exchangeRepository.upsert(entityOrEntities, ['coin'])
		return raw
	}
	...
	async getApiExchange(notFoundExchanges: BlockchainsSymbols[]) {
		...
		for await (const [blockchainName, exchangeName] of Object.entries(ExchangeNames)) {
	...
	async getExchange(): Promise<Record<BlockchainsSymbols, number>> {
		...
		return exchanges.reduce((acc, { coin, price }) => ({ ...acc, [coin]: price }), {})
	}
	...
	async sumQuerySelect(isSum = true, sumColumn = 'd.sum', blockchainColumn = 'd.blockchain') {
		const exchanges = await this.getExchange()
		const condition = `${sumColumn} * CASE ${blockchainColumn} ${Object.keys(BlockchainsSymbols)
