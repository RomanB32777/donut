import { format, parse, parseISO, addDays, endOfDay, startOfDay } from 'date-fns'
import { Between, MoreThanOrEqual } from 'typeorm'
import { allPeriodItemsTypes, periodItemsTypes } from 'types'

const intervals: Record<Exclude<allPeriodItemsTypes, 'all' | 'custom'>, number> = {
	yesterday: -1,
	today: 0,
	'7days': -7,
	'30days': -30,
...
const parseStringDateISO = (dateString: string, dateFormat?: string): Date => {
	const formatDate = format(parseStringDate(dateString, dateFormat), 'yyyy-MM-dd')
...
const getTimePeriod = ({
	...
	if (startDate && endDate) {
		return Between(startOfDay(parseStringDate(startDate)), endOfDay(parseStringDate(endDate)))
	}
...
export { intervals, dateTruncParams, parseStringDate, parseStringDateISO, getTimePeriod }
