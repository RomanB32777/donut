...
export const getRepositoryFields = <T>(repository: Repository<T>, excludedFields?: (keyof T)[]) => {
...
export const getDefaultValues = <T>(repository: Repository<T>, excludedFields?: (keyof T)[]) => {
	const excludedArr = excludedFields || []

	return repository.metadata.columns.reduce((acc, c) => {
		const name = c.propertyName as keyof T
		if (['string', 'number', 'boolean'].includes(typeof c.default) && !excludedArr.includes(name)) {
