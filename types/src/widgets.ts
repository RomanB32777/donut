...
interface IGoalData<FontType = string> extends IGoalDataBase, IGoalWidgetData<FontType> {
	id: string
	amountRaised: number
	isArchive: boolean
}
...
interface IAlertData<FontType = string, BannerType = string, SoundType = string> {
	id: string
	banner: BannerType
	messageColor: string
...
interface IEditAlertData extends Partial<Omit<IAlertData, 'creator'>>, IResetField {
	id: string
}
...
interface IStatData<FontType = string> extends IStatDataBase, IStatWidgetData<FontType> {
	id: string
}

interface IEditStatData extends Partial<Omit<IStatData, 'creator'>>, IResetField {
	id: string
}
